...
$serialize_data = $m->pack( $members_obj );
var_dump(bin2hex($serialize_data));
$members_obj = $m->unpack( $serialize_data );
var_dump($members_obj);

echo "\n--- testing reference to an obj ---\n";
$ref_members_obj = &$members_obj;
$serialize_data = $m->pack( $ref_members_obj );
var_dump(bin2hex($serialize_data));
$ref_members_obj = $m->unpack( $serialize_data );

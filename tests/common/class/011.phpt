--TEST--
MessagePacki: arrays (circular references)
--INI--
--FILE--
<?php
...
$serialize_data = $m->pack($arr_circ);
if (version_compare(PHP_VERSION, '5.3.0') < 0) {
    $arr_circ[6] = null;
}
var_dump(bin2hex($serialize_data));
$arr_circ = $m->unpack($serialize_data);
var_dump($arr_circ);
if (version_compare(PHP_VERSION, '5.3.0') < 0) {
    $arr_circ[6] = null;
}
...
var_dump($arr_asso);
$serialize_data = $m->pack($arr_asso);
var_dump(bin2hex($serialize_data));
$arr_asso = $m->unpack($serialize_data);
var_dump($arr_asso);

...
foreach ($data as $val) {
    echo "data: $val\n";
    echo "success:\n";
    $data = msgpacki_unserialize(pack('H*', $val), $status);
    var_dump($data);
    var_dump($status);
    echo "failure: (less)\n";
    $data = msgpacki_unserialize(pack('H*', substr($val, 1)), $status);
    var_dump($data);
    var_dump($status);
    echo "failure: (over)\n";
    $data = msgpacki_unserialize(pack('H*', $val . 'c0'), $status);
    var_dump($data);
    var_dump($status);
    echo "\n";
}

...
foreach ($data as $val) {
    echo "data: $val\n";
    echo "success:\n";
    $data = msgpacki_decode(pack('H*', $val), MSGPACKI_MODE_ORIGIN, $status);
    var_dump($data);
    var_dump($status);
    echo "failure: (less)\n";
    $data = msgpacki_decode(pack('H*', substr($val, 1)), MSGPACKI_MODE_ORIGIN, $status);
    var_dump($data);
    var_dump($status);
    echo "failure: (over)\n";
    $data = msgpacki_decode(pack('H*', $val . 'c0'), MSGPACKI_MODE_ORIGIN, $status);
    var_dump($data);
    var_dump($status);
    echo "\n";
}

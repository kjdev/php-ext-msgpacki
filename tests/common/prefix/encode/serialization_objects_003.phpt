--TEST--
Test msgpacki_encode() & msgpacki_decode() functions: objects (abstract classes)
--INI--
--FILE--
<?php
...
$serialize_data = msgpacki_encode($obj_extendName);
var_dump( bin2hex($serialize_data) );
$unserialize_data = msgpacki_decode($serialize_data);
var_dump( $unserialize_data );

$serialize_data = msgpacki_encode($obj_extendName->printClassName());
var_dump( bin2hex($serialize_data) );
$unserialize_data = msgpacki_decode($serialize_data);
var_dump( $unserialize_data );

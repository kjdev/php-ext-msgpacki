--TEST--
Test encode() & decode() functions: arrays (circular references)
--INI--
--FILE--
<?php
...
$serialize_data = encode($arr_circ);
var_dump(bin2hex($serialize_data));
$arr_circ = decode($serialize_data);
var_dump($arr_circ);
...
var_dump($arr_asso);
$serialize_data = encode($arr_asso);
var_dump(bin2hex($serialize_data));
$arr_asso = decode($serialize_data);
var_dump($arr_asso);

--TEST--
Test encode() & decode() functions: objects
--INI--
--FILE--
<?php
...
$serialize_data = encode( $members_obj );
var_dump(bin2hex($serialize_data));
$members_obj = decode( $serialize_data );
var_dump($members_obj);

echo "\n--- testing reference to an obj ---\n";
$ref_members_obj = &$members_obj;
$serialize_data = encode( $ref_members_obj );
var_dump(bin2hex($serialize_data));
$ref_members_obj = decode( $serialize_data );
